...
LOOKBACK_LEFT, LOOKBACK_RIGHT 모두 설정 가능
"""
import pandas as pd
import numpy as np
import json
...
class RSIDivergenceGridSearchFast:
    ...
    def grid_search(self, 
        ...
        Parameters:
        -----------
        lookback_left_range : list or range
            피벗 왼쪽 확인 범위 (예: [3, 5, 7])
        lookback_right_range : list or range
            피벗 오른쪽 확인 범위 (예: [1, 5, 10])
...
def process_single_combination(params, df_dict, fee_rate):
    ...
    # 다이버전스 감지
    bear_signals, bull_signals = detect_divergences_fast(df_dict, ll, lr)
...
def detect_divergences_fast(df_dict, lookback_left, lookback_right):
    """다이버전스 감지 (고속 버전)"""
    rsi = df_dict['rsi']
    high = df_dict['high']
    low = df_dict['low']
