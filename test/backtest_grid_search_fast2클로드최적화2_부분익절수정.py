...
def is_pivot_high_nb(rsi, idx, left, right):
    ...
    for i in range(idx - left, idx):
        if rsi[i] >= center:  # center보다 크거나 같으면 피봇 아님
            return False
...
def is_pivot_low_nb(rsi, idx, left, right):
    ...
    center = rsi[idx]
    for i in range(idx - left, idx):
        if rsi[i] <= center:  # center보다 작거나 같으면 피봇 아님
            return False
    for i in range(idx + 1, idx + right + 1):
        if rsi[i] <= center:  # center보다 작거나 같으면 피봇 아님
            return False
...
def get_signals_nb(rsi, high, low, ll, lr):
    ...
    for i in range(ll + 60, n - lr):  # ll+60부터 시작 (안전한 범위)
        # Bearish
...
def execute_trade_nb(close, high, low, sig_idx, is_bear, pp, hb, sl):
    ...
    # 🔥 수정: max 계산과 스탑로스/부분익절 체크를 동시에 진행
    for i in range(sig_idx, sig_idx + hb + 1):
        ...
        if sl > 0 and current_loss <= -sl:
            ...
            result = np.zeros(4)
            result[0] = total_pnl
            result[1] = max_loss  # 🔥 스탑로스 시점까지의 max_loss
            result[2] = max_profit  # 🔥 스탑로스 시점까지의 max_profit
            result[3] = 1.0  # 스탑로스 플래그
            return result
        
        # 부분익절 체크
        if not partial_closed and current_profit >= pp:
            partial_pnl = current_profit * 0.5  # 50% 포지션 수익
            partial_closed = True
    
    # 🔥 수정: 보유기간 종료 - 부분익절 여부에 따라 다르게 계산
    ...
    # 부분익절 했으면 남은 50%만, 안 했으면 100%
    if partial_closed:
        remaining_pnl = exit_profit * 0.5  # 남은 50% 포지션
    else:
        remaining_pnl = exit_profit  # 100% 포지션
