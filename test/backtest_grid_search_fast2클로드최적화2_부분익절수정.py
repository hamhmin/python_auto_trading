"""
RSI Divergence 그리드 서치 (정상 청산만 수정!)
- 부분익절: current_profit 유지 ✅
- 정상 청산: partial_closed 체크 ✅
- 스탑로스: 원본 유지
"""
import pandas as pd
import numpy as np
...
def execute_trade_nb(close, high, low, sig_idx, is_bear, pp, hb, sl):
    ...
    for i in range(sig_idx, sig_idx + hb + 1):
        ...
        if not partial_closed and current_profit >= pp:
            partial_pnl = current_profit * 0.5  # ← current_profit 유지!
            partial_closed = True
    
    # ✅ 정상 청산 (유일한 수정!)
    exit_price = close[sig_idx + hb]
    if is_bear:
        exit_profit = ((entry_price - exit_price) / entry_price) * 100
    else:
        exit_profit = ((exit_price - entry_price) / entry_price) * 100
    
    if partial_closed:
        remaining_pnl = exit_profit * 0.5  # 부분익절 발동: 50%
    else:
        remaining_pnl = exit_profit  # 부분익절 안 됨: 100% ✅
    ...
    for pp_idx in range(n_pp):
        ...
        for hb_idx in range(n_hb):
            ...
            for sl_idx in range(n_sl):
                ...
                result[14] = max_loss_val
                result[15] = sum_max_profit
...
def process_batch_ultra(batch_data, close, high, low, rsi, pp_arr, hb_arr, sl_arr, fee_rate):
    ...
    for r in numba_results:
        ...
        avg_max_profit = sum_max_profit / n_trades
        ...
        if max_loss_val != 0:
            ...
            ultimate_rank = expected_return / abs(max_loss_val)
        else:
            ultimate_rank = 0.0
        ...
        batch_results.append({
            ...
            'worst_trade_pnl': round(min_pnl, 4),
            'avg_max_loss': round(avg_max_loss, 4),
            'avg_max_profit': round(avg_max_profit, 4),
            ...
            'max_lvg+분할=pnl': round(leverage_calc, 4),
            '손익비': round(loss_profit_per, 4)
        })
...
def main():
    if len(sys.argv) < 2:
        print("사용법: python backtest_정상청산만수정.py <data.json>")
        return
    ...
    if all_results:
        ...
        filename = f"backtest_정상청산수정_{datetime.now().strftime('%m%d_%H%M%S')}.csv"
        ...
        for metric, name in [
            ('kelly_criterion', '켈리 지수'),
            ('expectancy_score', '수익 안정성'),
            ('ultimate_rank', '통합 점수'),
            ('max_lvg+분할=pnl', '최대 레버리지+분할'),
            ('손익비', '손익비'),
            ('avg_pnl', 'avg_pnl')
        ]:
            best_metric = positive_results.nlargest(1, metric).iloc[0]
            print(f"\n   {name} 최고: {best_metric[metric]:.4f}")
            print(f"   → ll={best_metric['lookback_left']}, lr={best_metric['lookback_right']}, "
                  f"pp={best_metric['partial_profit']}, hb={best_metric['hold_bars']}, sl={best_metric['stop_loss']}")
            print(f"   → 총수익: {best_metric['total_pnl']:+.2f}%, 승률: {best_metric['win_rate']:.1f}%")
